...
function of_ajax_callback() 
{
	...
	if ( in_array( 'frontend_builder/frontend_builder.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
		...
		$options = array (
			'bottom_margin' => $data['fb_bmargin'],
			'high_rezolution_width' => $data['content_width'],
			'high_rezolution_margin' => $data['fb_hres_c'],
			'med_rezolution_width' => $data['fb_mres_w'],
			'med_rezolution_margin' => $data['fb_mres_c'],
			'med_rezolution_hide_sidebar' => ( $data['fb_mres_s'] == 1 ) ? 'true' : 'false',
			'low_rezolution_width' => $data['fb_lres_w'],
			'low_rezolution_margin' => $data['fb_lres_c'],
			'low_rezolution_hide_sidebar' => ( $data['fb_lres_s'] == 1 ) ? 'true' : 'false',
			'main_color' => $data['theme_color'],
			'light_main_color' => $data['theme_color_light'],
			'text_color' => $data['text_color'],
			'title_color' => $data['headers_color'],
			'dark_back_color' => $data['bg_default_color'], 
			'light_back_color' => $data['bg_pale_color'],
			'dark_border_color' => $data['bg_default_color'],
			'light_border_color' => $data['bg_pale_color']
		);
	...
	elseif ($save_type == 'demo_install') {
		include_once(dirname(__FILE__) . '/../demo/demo-fbuilder-options.php');
		include_once(dirname(__FILE__) . '/../demo/demo-fbuilder-pages.php');
		include_once(dirname(__FILE__) . '/../demo/demo-posts.php');
		include_once(dirname(__FILE__) . '/../demo/demo-terms.php');
		include_once(dirname(__FILE__) . '/../demo/demo-revsliders.php');
		include_once(dirname(__FILE__) . '/../demo/demo-revslides.php');
		include_once(dirname(__FILE__) . '/../demo/demo-revcss.php');
		set_transient('PortalWP_Demo_Installation', 'installed');
		die('1'); //demo installed
	}
	elseif ($save_type == 'demo_remove') {
		set_transient('PortalWP_Demo_Remove', 'remove');
		die('1'); //options remove
	}
